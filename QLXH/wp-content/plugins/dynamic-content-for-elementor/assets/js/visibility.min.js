...
const triggerEventCallback=(event)=>{event.preventDefault();const triggerFun=isToggle?toggle:show;if(hideOther){hideOtherElements(triggerFun)}else{triggerFun()}}
if(triggerElement){$(triggerElement).on(triggerEvent,triggerEventCallback)}
if(showOnLoad){setTimeout(()=>{$e.removeClass('dce-visibility-element-hidden');$e.hide();if(showOnLoadAnimation==='slide'){$e.slideToggle(transitionDelay,afterShow)}else if(showOnLoadAnimation==='fade'){$e.fadeToggle(transitionDelay,afterShow)}else{$e.toggle();afterShow()}},showOnLoadDelay)}};$(window).on('elementor/frontend/init',()=>{elementorFrontend.hooks.addAction('frontend/element_ready/global',initVisibility)})})