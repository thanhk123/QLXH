...
class ThePlus_Search_Filter extends Widget_Base {
    ...
    protected function register_controls() {
        ...
        $this->start_controls_section('Autocomplate_styling',
            [
                'label'=>esc_html__('Autocomplate','theplus'),
                'tab'=>Controls_Manager::TAB_STYLE			
            ]
        );
        ...
        $this->add_control('FRiconHCR',
            [
                ...
                'type'=>Controls_Manager::COLOR,
                'selectors'=>[
                    '{{WRAPPER}} .tp-search-filter .tp-filter-meta span.tp-tag-reset:hover .tp-tag-link i'=>'color:{{VALUE}}'
                ]
            ]
        );
        $this->add_group_control(Group_Control_Background::get_type(),
            [
                'name'=>'FRHBGCr',
                'types'=>['classic','gradient'],
                'selector'=>'{{WRAPPER}} .tp-search-filter .tp-filter-meta span.tp-tag-reset:hover',
            ]
        );
        $this->add_group_control(Group_Control_Border::get_type(),
            [
        ...
        $this->add_responsive_control('FRHBrs',
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .tp-search-filter .tp-filter-meta span.tp-tag-reset:hover'=>'border-radius:{{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};'
                ]
            ]
        );
        $this->add_group_control(Group_Control_Box_Shadow::get_type(),
    ...
    protected function tp_rating($Repeater, $name){
        ...
        $op .= '<div class="tp-star-rating" data-tpprefix="'.esc_attr($TPPrefix).'">';
            for ($i=1; $i<=5; $i++) {
                $value = 6-$i;
                $op .= '<input type="radio" name="'.esc_attr($name).'" class="stars-'.esc_attr($value).'" id="'.esc_attr($TPPrefix).esc_attr($value).'" value="'.esc_attr($value).'" data-title="'.esc_attr($value).'" />';
